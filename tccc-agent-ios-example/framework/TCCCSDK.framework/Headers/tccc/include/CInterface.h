...
#    define TCCCSDK_API extern __declspec(dllexport)
#  else
#    define TCCCSDK_API extern "C" __declspec(dllexport)
#  endif
#  define TCCCSDK_CALL __cdecl
#else
#  define TCCCSDK_API extern "C"
#  define TCCCSDK_CALL __cdecl
#endif
...
TCCCSDK_API void* TCCC_GetTCCCInstance(void* context);
TCCCSDK_API void TCCC_DestroyTCCCInstance();

TCCCSDK_API void TCCC_AddCallback(TCCC_OnErrorHandler onError, TCCC_OnWarningHandler onWarning,
                                  ...
                                  TCCC_OnNetworkQualityHandler onNetworkQuality, TCCC_OnConnectionLost onConnectionLost,
                                  TCCC_OnTryToReconnect onTryToReconnect,
                                  TCCC_OnConnectionRecovery onConnectionRecovery);
TCCCSDK_API void TCCC_RemoveCallback();

TCCCSDK_API void TCCC_Login(const char* userId, const char* password, uint32_t sdkAppId, int loginType,
                            TCCC_OnLoginHandler callback);
TCCCSDK_API void TCCC_Logout(TCCC_OnLogoutHandler callback);

TCCCSDK_API void TCCC_CheckLogin(TCCC_OnCheckLoginHandler callback);
TCCCSDK_API void TCCC_ResetSip(bool isNeedCallReLogin);

TCCCSDK_API void TCCC_Call(const tccc::TCCCStartCallParams& param, TCCC_OnStartCallHandler callback);
TCCCSDK_API void TCCC_Terminate();
TCCCSDK_API void TCCC_Answer(TCCC_OnAnswerHandler callback);
TCCCSDK_API void TCCC_SendDTMF(const char digit, TCCC_OnSendDTMFHandler callback);
TCCCSDK_API void TCCC_Mute();
TCCCSDK_API void TCCC_Unmute();
TCCCSDK_API const char* TCCC_GetSDKVersion();
TCCCSDK_API void TCCC_CallExperimentalAPI(const char* commandType, const char* jsonStr);
TCCCSDK_API void TCCC_StartPlayMusic(const char* path, int loopCount);
TCCCSDK_API void TCCC_StopPlayMusic();
TCCCSDK_API void TCCC_SetLogLevel(int level);
TCCCSDK_API void TCCC_SetConsoleEnabled(bool enabled);
TCCCSDK_API void TCCC_EnableAudioVolumeEvaluation(uint32_t interval, bool enable_vad);
TCCCSDK_API void TCCC_SetLogDirectory(const char* path);
// 设备
TCCCSDK_API void TCCC_GetDeviceManager();
TCCCSDK_API void TDEVICE_SetAudioCaptureVolume(int volume);
TCCCSDK_API int TDEVICE_GetAudioCaptureVolume();
TCCCSDK_API void TDEVICE_SetAudioPlayoutVolume(int volume);
TCCCSDK_API int TDEVICE_GetAudioPlayoutVolume();

TCCCSDK_API void TDEVICE_SetAudioRoute(int route);

// for test ,callback的message返回token
TCCCSDK_API void TCCC_GenTestTokenByUser(const char* secretId, const char* secretKey, const char* userId,
